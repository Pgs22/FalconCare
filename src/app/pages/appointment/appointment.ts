...
export class AppointmentComponent implements OnInit {
  ...
  openOdontogram(appointment: Appointment): void {
    this.appointmentService.openAppointment(appointment.id).subscribe({
      next: (response: unknown) => {
        const payload = this.asRecord(response);
        const appointmentPayload = this.asRecord(payload?.['appointment']);
        const patientId = this.toNumberOrNull(
          appointmentPayload?.['patientId']
            ?? this.asRecord(appointmentPayload?.['patient'])?.['id']
            ?? this.getAppointmentPatientId(appointment)
        );
